...
class DataCleanser:
    """도로명 주소와 지번 주소를 카카오 API에 전달하기 전 클렌징하는 클래스"""
    ...
    def __init__(self, df: pd.DataFrame, district_name: str) -> None:

        self.__df = df
        self.__rn_adrs_df = None
        self.__lno_adrs_df = None
    ...
    def replace_location_strings(self):
        """도로명주소와 지번주소 데이터의 저품질 데이터를 클렌징하는 함수"""
        ...
        self.__df["lno_adrs"] = (
            self.__df["lno_adrs"]
            .str.replace(pat=r"\([ㄱ-ㅣ가-힣0-9\s.:]+\)", repl="", regex=True)
            .str.replace("  ", " ")
        )

    def delete_ambiguous_location(self):
        """좌표 주소를 얻을 필요가 없는 행들을 제거하는 함수"""

        ### 상담문의나 내용확인 불가 항목 제거
        elems = ["상담문의", "내용확인불가"]
        cond = ~(
            (self.__df["incd_ass_nm"].isin(elems))
            & (self.__df["end_cl_nm"] == "비출동종결")
            & (self.__df["stt_end_nm"] == "조치없이 종결")
        )
        self.__df = self.__df[cond]
        ...
        cond = ~(
            (self.__df["ass_cl_nm"] == "기타경찰업무")
            & (self.__df["incd_ass_nm"] == "상담문의")
            & (self.__df["end_cl_nm"] == "비출동종결")
        )
        self.__df = self.__df[cond]
        ...
        cond = ~(
            (self.__df["ass_cl_nm"] == "타기관_기타")
            & (self.__df["incd_ass_nm"] == "내용확인불가")
            & (self.__df["end_cl_nm"] == "비출동종결")
        )
        self.__df = self.__df[cond]
    ...
    def delete_other_district(self):
        """지역일 일치하는 하지 않는 행들을 제거하는 함수"""

        del_districts = [
            district_iter for district_iter in self.ALL_DISTRICTS if self.__district_name not in district_iter
        ]
